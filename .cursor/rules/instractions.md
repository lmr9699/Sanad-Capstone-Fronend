...
**See `.cursor/rules/HARD_RULES.md` for complete rules. Summary:**

1. **DO NOT** invent new features or screens
2. **DO NOT** change design, wording, spacing, or colors without approval
3. **DO NOT** delete, rename, or reorganize files/folders without asking
4. **DO NOT** refactor/restructure without proposing a plan first
5. **DO NOT** commit secrets — use .env (must be gitignored)

**Always ask before making changes that violate these rules.**
...
- run "npm run reset-project" to clear the default settings
...
├── api/ → API functions (used by React Query)
